...
x_train = np.linspace(0, 10, 1000).reshape(1, -1) # np.array([[0, 0], [0, 1], [1, 0], [1, 1]]).reshape(2, 4)
y_train = f(x_train).reshape(1, -1)
x_test = np.linspace(0, 10, 10).reshape(1, -1) # np.array([[0, 0], [0, 1], [1, 0], [1, 1]]).reshape(2, 4)
y_test = f(x_test).reshape(1, -1)

hidden = 2
regression_network = nn.Network(loss=nn.mse, loss_prime=nn.mse_prime, learn_rate=0.001, epochs=500)
# input layer
regression_network.add_hidden_layer(layer.LinearLayer(1, hidden))
